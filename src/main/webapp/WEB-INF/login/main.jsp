	...
	<nav class="navbar navbar-expand-lg navbar-light bg-light fixed-top">
		...
		<div class="collapse navbar-collapse">
			<ul class="navbar-nav ml-auto">
				<li class="nav-item">
					<a href="/interests/${thisUser.id}" class="nav-link">My Interests</a>
				</li>
				...
				<div class="container" style="margin: 30px 40px 20px 20px;">
					...
					<c:if test="${post.interested_users.contains(thisUser) == false}">
						<form method="get" action="/post/${post.id}/interested">
							<button type="submit" style="border: 0px; background-color: #ff5200; border-radius: 5px;">I'm Interested</button>
						</form>
					</c:if>
				</div>
			</div>
